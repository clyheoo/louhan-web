...
function nomShowWaiting(nominations) {
    ...
    list.innerHTML = '<div class="space-y-2">' + nominations.map(n =>
        ...
        '<div class="w-9 h-9 bg-slate-900 text-white rounded-lg flex items-center justify-center text-xs font-bold shrink-0">' + n.nomor_tank + '</div>' +
...
function nomSubmit() {
    ...
    list.innerHTML = selectedTanks.map(function(t) {
        return '<div class="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100">' +
            '<div class="w-10 h-10 bg-slate-900 text-white rounded-lg flex items-center justify-center text-sm font-bold shrink-0">' + t.nomor_tank + '</div>' +
...
function renderLiveTable() {
    ...
    empty.classList.add('hidden');
    body.innerHTML = appData.my_scores.map(s => { const t = s.ikan; const nd = s.nilai_detail||{}; const fmt = (obj,keys) => keys.map(k=>nd[obj]?.[k]||'-').join('/'); let defHtml = ''; ['head','face','body','finnage'].forEach(p => { const raw = s['raw_'+p+'_penalty']; if (raw && Array.isArray(raw) && raw[0]!=='0' && raw.length>0) defHtml += '<span class="inline-block bg-red-100 text-red-700 text-[8px] font-bold px-1 py-0 rounded mb-0.5">'+raw.join(', ')+'</span>'; }); if (!defHtml) defHtml = '<span class="text-slate-300">-</span>'; const toG = s.submitted_to_grand; return '<tr class="hover:bg-amber-50/50"><td class="px-2 py-2 border-r font-bold text-slate-800 text-center bg-slate-50 sticky left-0 z-10 text-xs">'+(t?t.nomor_tank:'-')+'</td><td class="px-2 py-2 border-r"><div class="font-bold text-[10px]">'+(t?.kategori||'-')+'</div><div class="text-[9px] text-blue-600 font-bold">KLS:'+(s.kelas||'-')+'</div></td><td class="px-2 py-2 border-r text-center font-mono font-bold text-blue-700 bg-blue-50/30">'+(nd.overall?.impression||'-')+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('head',['size','bentuk'])+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+(nd.face?.face||'-')+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('body',['bentuk','proporsi','pangkal'])+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('marking',['fullness','contrast','bentuk'])+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('pearl',['shinning','fullness','bentuk'])+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('color',['komposisi','kecerahan','fullness'])+'</td><td class="px-2 py-2 border-r text-center font-mono text-[10px]">'+fmt('finnage',['bentuk','kecerahan'])+'</td><td class="px-2 py-2 border-r text-left align-top min-w-[100px] whitespace-normal">'+defHtml+'</td><td class="px-2 py-2 text-center">'+'<button onclick="lihatDetail('+s.id+')" class="px-2 py-1 bg-slate-700 text-white text-[9px] font-bold rounded hover:bg-slate-800 flex items-center gap-1 mx-auto"><svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>Detail</button>'+'</td></tr>'; }).join('');
}
